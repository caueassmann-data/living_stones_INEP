...
from src.evaluate import evaluate_by_subgroup, evaluate_on_test, evaluate_ranking, export_global_importance
...
def train_level(level: str, max_rows: int | None = None) -> dict[str, Any]:
    ...
    if temporal_test_mask.sum() >= 50 and temporal_train_mask.sum() >= 50:
        ...
        temporal_metrics = _fit_and_score(
            winner, Xt_train, yt_train, Xt_test, yt_test, hr_test, numeric_columns, categorical_columns
        )
